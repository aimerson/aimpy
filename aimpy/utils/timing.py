...
def decimal_years(DD,MM,YYYY):
    DATE = datetime.datetime(YYYY,MM,DD)
    START = datetime.datetime(YYYY,1,1)
    END = datetime.datetime(YYYY,12,31)
    full_year = (END-START).total_seconds()
    part_year = (DATE-START).total_seconds()
    years = float(YYYY)+(float(part_year)/float(full_year))
    return years
