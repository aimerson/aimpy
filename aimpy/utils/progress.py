...
class Progress:
    ...
    def status_bar(self, size=60):   
        """Return a status bar. Optionally a task name can be passed in to be
        included as well."""
        status = str(self)
        j = self.history[-1][0]
        x = int(size*j/self.total_work)
        indChar = "#"
        indChar = u"█"
        line = "{} [{}{}] {}/{}".format(status, indChar*x, "."*(size-x), j, self.total_work)
        return line
    ...
    def print_status_bar(self, size=60):
        """Write a status line to stdout. If the work is completed then the
        cursor is moved to the next line, but otherwise it is kept on the
        current one. Optionally a task name can be passed in to be displayed
        as well."""
        if self.done():
            ...
            eol = "\r"
        line = self.status_bar(size=size)
        line_length = 2*size
        line = line[:line_length]
        sys.stdout.write("%s%s%s" % (line, " " * (line_length-len(line)), eol))
        sys.stdout.flush()
